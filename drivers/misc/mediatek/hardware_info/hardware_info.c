...
struct hardware_info current_sub_lcm_info =
{
	"unknow","unknow","unknow","unknow",
};
...
struct hardware_info current_sub_tp_info =
{
	"unknow","unknow","unknow","unknow",
};
...
/* prize mod by wanwen 20250717, show fuelgauge firmware version start */
...
#endif
/* prize mod by wanwen 20250717, show fuelgauge firmware version end */
...
/* pri LAX10-192 added by xuejian 20240409 begin*/
static void  dev_get_current_sub_lcm_info(char *buf)
{
	char *p = buf;
	HW_PRINT("dev_get_current_sub_lcm_info");
	if(strcmp(current_sub_lcm_info.chip,"unknow") == 0)
	 	return ;

	p += sprintf(p, "\n[Sub LCM]:\n");
	p += sprintf(p, "  chip:%s\n", current_sub_lcm_info.chip);
	p += sprintf(p, "  id:%s\n", current_sub_lcm_info.id);
	p += sprintf(p, "  vendor:%s\n",current_sub_lcm_info.vendor);
	p += sprintf(p, "  more:%s\n", current_sub_lcm_info.more);
...
/* pri LAX10-192 added by xuejian 20240410 begin*/
static void  dev_get_current_sub_tp_info(char *buf)
{
	char *p = buf;
	HW_PRINT("dev_get_current_sub_tp_info");
	if(strcmp(current_sub_tp_info.chip,"unknow") == 0)
	 	return ;

	p += sprintf(p, "\n[Sub Touch Panel]:\n");
	p += sprintf(p, "  chip:%s\n", current_sub_tp_info.chip);
	p += sprintf(p, "  id:%s\n", current_sub_tp_info.id);
	p += sprintf(p, "  vendor:%s\n",current_sub_tp_info.vendor);
	p += sprintf(p, "  more:%s\n", current_sub_tp_info.more);
...
//prize add by wangfei for line_motor 20221027 end
...
static void dev_get_current_battery_info(char *buf)
{
	...
	p += sprintf(p, "\n[battery]:\n");
/* prize mod by wanwen 20250717, show fuelgauge firmware version start */
    p += sprintf(p, "  chip:%s\n", current_battery_info.chip);
	p += sprintf(p, "  vendor:%s\n", current_battery_info.vendor);
	p += sprintf(p, "  more:%s\n", current_battery_info.more);
/* prize mod by wanwen 20250717, show fuelgauge firmware version end */
...
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_LPDDR_INFO)
	char *p = buf;
//prize modify by luoyu for flashlist Replace nodes 2020305 start
	int ret = 0;
#if !IS_ENABLED(CONFIG_PRI_ODM)
	int i = 0;
#endif
//prize modify by luoyu for flashlist Replace nodes 2020305 end
	HW_PRINT("dev_get_current_flash_lpddr_info");
	
	ret = get_lpddr_used_index();
	HW_PRINT("get_lpddr_used_index is %d, sizeof Cust_emmc_support is %lu", ret, sizeof(Cust_emmc_support) / sizeof(Cust_emmc_support[0]));
...
//prize modify by luoyu for flashlist Replace nodes 2020305 end
	...
	char uemcp_pn_info[][50] = {
		{"MICRON MT30AZZZCDAZTPWL_031W_16C"},
		{"MICRON MT30AZZZCDAZTPWL_031W_16C"},
		{"MICRON MT30AZZZCDAZTPWL_031W_16C"},
		{"MICRON MT30AZZZCDAZTPWL_031W_16C"},
		{"MICRON MT30AZZZCDAZTPWL_031W_16C"},
...
/*	p += sprintf(p, "  chip:%s\n",    AUDIO_PARA_CHIP);
	...
	p += sprintf(p, "  ver:%s\n",     AUDIO_PARA_VERSION);
*/
	p += sprintf(p, "%s", databuf);

//begin add audio PA type
	p += sprintf(p, "PA chip:%s\n",    "Dual AW87565");
//end add audio PA type
...
//prize add by luoyu for flashlist Replace nodes 2020305 start
static void dev_get_current_flashlist_info(char *buf)
{
	char *p = buf;
	int i, ret = 0;
	HW_PRINT("dev_get_current_flash_lpddr_info");
...
//prize add by luoyu for flashlist Replace nodes 2020305 end
...
static ssize_t hardware_info_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	/* pri LAX10-192 added by xuejian 20240409 begin*/
	dev_get_current_sub_lcm_info(buf + len);
	/* pri LAX10-192 added by xuejian 20240409 end*/
	...
	/* pri LAX10-192 added by xuejian 20240410 begin*/
	dev_get_current_sub_tp_info(buf + len);
	/* pri LAX10-192 added by xuejian 20240410 end*/
...
//prize add by luoyu for flashlist Replace nodes 2020305 start
static ssize_t hw_info_flashread_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	len = 0;
	HW_PRINT("hardware_info_flashshow=======");

	dev_get_current_flashlist_info(buf + len);
...
//prize add by luoyu for flashlist Replace nodes 2020305 start
static DEVICE_ATTR_RO(hw_info_flashread);
//prize add by luoyu for flashlist Replace nodes 2020305 end
...
#endif
#if IS_ENABLED(CONFIG_PRI_TYPEC_CHG_DET)//prize added by wangmd,do not detect P/N at power test,start
...
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO_LAVA_CUST)
...
static void hwinfo_lava_get_camera_info(char *buf)
{
	...
	ptr += sprintf(ptr, "\nCamera Info:\n");
	for (cam_idx = 0; cam_idx < 4; ++cam_idx) {
		lava_caminfo[cam_idx].chip = current_camera_info[cam_idx].chip;
		lava_caminfo[cam_idx].supplier = current_camera_info[cam_idx].vendor;
...
static void hwinfo_lava_get_lcm_tp_info(char *buf)
{
	...
	ptr += sprintf(ptr, "\nMain LCD Info:\n");
	ptr += sprintf(ptr, "%s:%s\n", current_lcm_info.vendor, current_lcm_info.chip);

	ptr     += sprintf(ptr, "\nMain TP Version:\n");
	ptr     += sprintf(ptr, "%s:%s\n", current_lcm_info.vendor, current_tp_info.chip);

	ptr += sprintf(ptr, "\nSub LCD Info:\n");
	ptr += sprintf(ptr, "%s:%s\n", current_sub_lcm_info.vendor, current_sub_lcm_info.chip);

	ptr     += sprintf(ptr, "\nSub TP Version:\n");
	ptr     += sprintf(ptr, "%s:%s\n", current_sub_lcm_info.vendor, current_sub_tp_info.chip);
...
//prize add by luoyu for flashlist Replace nodes 2020305 start
	if (device_create_file(hardware_info_dev, &dev_attr_hw_info_flashread) < 0)
		HW_ERROR("Failed to create device file(%s)!",dev_attr_hw_info_flashread.attr.name);
//prize add by luoyu for flashlist Replace nodes 2020305 end
...
/* drv added by zhangxia for audio params verision 20240530 start */
	...
	schedule_delayed_work(&audio_firmware_delayed_work, msecs_to_jiffies(8000));
/* drv added by zhangxia for audio params verision 20240530 end */
