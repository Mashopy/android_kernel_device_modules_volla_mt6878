...
//begin add nfc hw version compatible solution
#include "../../hardware_info/hardware_info.h"

extern struct hardware_info current_nfc_info;
static bool hw_info_flag = false;
//end add nfc hw version compatible solution
...
//begin add nfc hw version compatible solution
/**hci cmd like:
**Rx 60 00 1f 01 01 20 02 1a 07 01 02 06 17 50 03 00 01 00 44 25 c9 00 00 5b d7 00 00 00 00 00 00 16
**from the print we know, every complete HCI cmd consists of TWO i2c rcv:3 bytes HEAD + payload.
**3byte core-reset-ntf head: 2bytes head "60 00" and 1byte payload length.
**st21nfc_dev_read(0) coosea nfc count=3, ret=3
**st21nfc_dev_read(head) 0x60, 0x0 , 0x1F
**payload data: 1F bytes, the sixth byte is hw-version.
**st21nfc_dev_read(0) coosea nfc count=31, ret=31
**st21nfc_dev_read(head) 0x1 , 0x1 , 0x20, 0x2 , 0x1A, 0x7 , 0x1 , 0x2 , 0x6
*/
	uint8_t hw_version;
	static bool prv_head_flag = false;

	if (false == hw_info_flag) {
		//pr_info("%s(0) coosea nfc count=%zu, ret=%d\n", __func__, count, ret);

		//check the previous head.
		if (ret == count && 0x03 == ret) {
			//pr_info("%s(head) 0x%-2X, 0x%-2X, 0x%-2X", __func__, st21nfc_dev->buffer[0], st21nfc_dev->buffer[1], st21nfc_dev->buffer[2]);

			if(0x60 == st21nfc_dev->buffer[0] && 0x00 == st21nfc_dev->buffer[1] && 0x1F == st21nfc_dev->buffer[2]) {
				//make sure the previous cmd is the 60 00 1F
				prv_head_flag = true;
			} else {
				prv_head_flag = false;
			}
		} else if (prv_head_flag && ret == count && 0x1F == ret) {
		//check curren payload
			/*pr_info("%s(head) 0x%-2X, 0x%-2X, 0x%-2X, 0x%-2X, 0x%-2X, 0x%-2X, 0x%-2X, 0x%-2X, 0x%-2X \n", __func__,
				st21nfc_dev->buffer[0], st21nfc_dev->buffer[1], st21nfc_dev->buffer[2], st21nfc_dev->buffer[3], st21nfc_dev->buffer[4],
				st21nfc_dev->buffer[5], st21nfc_dev->buffer[6], st21nfc_dev->buffer[7], st21nfc_dev->buffer[8]);
			*/

			hw_version = st21nfc_dev->buffer[5];
			pr_info("%s(1) coosea nfc hw_version=0x%-2X\n", __func__, hw_version);

			if(0x06 == hw_version) {
				hw_info_flag = true;
				strcpy(current_nfc_info.chip, "ST54L");
			} else if(0x07 == hw_version) {
				hw_info_flag = true;
				strcpy(current_nfc_info.chip, "ST21L");
			} else {
				sprintf(current_nfc_info.chip, "unknown:0x%-2X", hw_version);
				pr_warn("%s()error: coosea unknown hw version=0x%-2X\n", __func__, hw_version);
			}
			strcpy(current_nfc_info.id, "0x08");
			strcpy(current_nfc_info.vendor, "STMicroelectronics");
			strcpy(current_nfc_info.more, "i2c3");
			//pr_info("%s(2) coosea nfc chip=0x%-2X\n", __func__, hw_version);
		}
	}
//end add nfc hw version compatible solution
