...
/*
  new item ,add here
*/
#if IS_ENABLED(CONFIG_TOUCH_HOLSTER_MODE)
	HOLSTER_MODE,
#endif
	MAX
};
...
*/
#if IS_ENABLED(CONFIG_TOUCH_HOLSTER_MODE)
static ssize_t holster_show(struct device *dev,struct device_attribute*attr, char *buf)
{
	int count;
	count = sprintf(buf, "Node Func: %s, state = %s\n",local_common_node->node_array[HOLSTER_MODE].name,local_common_node->node_array[HOLSTER_MODE].state?"On":"Off");
	return count;
}

static ssize_t holster_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	int error;
	unsigned int temp;
	error = kstrtouint(buf, 10, &temp);
	if(error < 0)
		goto err;
	if(local_common_node->node_array[HOLSTER_MODE].set){
		local_common_node->node_array[HOLSTER_MODE].state = temp;
		local_common_node->node_array[HOLSTER_MODE].set(temp);
	}else
		printk("node func %s is null!! \r\n",local_common_node->node_array[HOLSTER_MODE].name);
err:
	return count;
}
#endif
...
static ssize_t gesturesub_show(struct device *dev,struct device_attribute*attr, char *buf)
{
	int count;
	count = sprintf(buf, "Node Func: %s, state = %s\n",local_common_node->node_array[GESTURE_SUB].name,local_common_node->node_array[GESTURE_SUB].state?"On":"Off");
	return count;
}

static ssize_t gesturesub_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	int error;
	unsigned int temp;
	error = kstrtouint(buf, 10, &temp);
	if(error < 0)
		goto err;
	if(local_common_node->node_array[GESTURE_SUB].set){
		local_common_node->node_array[GESTURE_SUB].state = temp;
		local_common_node->node_array[GESTURE_SUB].set(temp);
	}else
		printk("node func %s is null!! \r\n",local_common_node->node_array[GESTURE_SUB].name);
err:
	return count;
}
...
static DEVICE_ATTR(gesturesub, S_IRUGO|S_IWUSR, gesturesub_show, gesturesub_store);
#if IS_ENABLED(CONFIG_TOUCH_HOLSTER_MODE)
static DEVICE_ATTR(holster, S_IRUGO|S_IWUSR, holster_show, holster_store);
#endif
...
static const struct attribute *common_node_event_attr[] = {
		...
		&dev_attr_gesturesub.attr,
	//	&dev_attr_supertorch.attr,
#if IS_ENABLED(CONFIG_TOUCH_HOLSTER_MODE)
		&dev_attr_holster.attr,
#endif
/*
  new item ,add here
*/
...
EXPORT_SYMBOL(pri_common_node_register);

void pri_common_node_show_register(char* name,bool(*hbm_set)(void))
{
	int i;
	for(i = 0;i < MAX;i++)
	{
...
EXPORT_SYMBOL(pri_common_node_show_register);
