...
/* pri x91daria-chg modify by suwenwei 20250225 */
...
#define upm_err(fmt, ...)								\
do {											\
		pr_err("[%d]:" fmt, __LINE__, ##__VA_ARGS__);\
} while(0);

#define upm_info(fmt, ...)								\
do {											\
		pr_info("[%d]:" fmt, __LINE__, ##__VA_ARGS__);\
} while(0);
/************************************************************************/
typedef enum {
	CHG_SW_CAP_MODE,
...
static int upm6722_check_charge_enabled(struct upm6722 *upm, bool *enable)
{
	...
	if ((!ret) && (val & UPM6722_CHG_EN_MASK) &&
			(val1 & UPM6722_CONV_ACTIVE_STAT_MASK)) {
		*enable = true;
	} else {
		*enable = false;
	}
...
static int upm6722_get_adc_data(struct upm6722 *upm, adc_channel_t channel,  int *result)
{
	...
	if (channel == UPM_ADC_IBUS && sval >= 3500) {
		upm_err("UPM6722 ibus ADC abnormal data = %d\n", sval);
		msleep(40);

		ret = upm6722_read_byte(upm, ADC_REG_BASE + (UPM_ADC_IBUS << 1), &val_h);
		if (ret < 0) {
			return ret;
		}

		ret = upm6722_read_byte(upm, ADC_REG_BASE + (UPM_ADC_IBUS << 1) + 1, &val_l);
		if (ret < 0) {
			return ret;
		}
		val = (val_h << 8) | val_l;
		sval = val * UPM6722_IBUS_ADC_SW_CAP_LSB / UPM6722_IBUS_ADC_SW_CAP_PRECISION + UPM6722_IBUS_ADC_SW_CAP_BASE + UPM6722_IBUS_ADC_SW_CAP_OFFSET;
		upm_err("iBUS ADC correction data = %d\n", sval);
	}

	if (channel == UPM_ADC_VBAT && sval >= 4600) {
		upm_err("UPM6722 VBAT ADC abnormal data = %d\n", sval);

		msleep(40);

		ret = upm6722_read_byte(upm, ADC_REG_BASE + (UPM_ADC_VBAT << 1), &val_h);
		if (ret < 0) {
			return ret;
		}

		ret = upm6722_read_byte(upm, ADC_REG_BASE + (UPM_ADC_VBAT << 1) + 1, &val_l);
		if (ret < 0) {
			return ret;
		}
		val = (val_h << 8) | val_l;
		sval = val * UPM6722_VBAT_ADC_LSB / UPM6722_VBAT_ADC_PRECISION + UPM6722_VBAT_ADC_BASE + UPM6722_VBAT_ADC_OFFSET;
		upm_err("VBAT ADC correction data = %d\n", sval);
	}

	if (channel == UPM_ADC_IBAT && sval >= 6500) {
		upm_err("UPM6722 iBAT ADC abnormal data = %d\n", sval);

		msleep(40);

		ret = upm6722_read_byte(upm, ADC_REG_BASE + (UPM_ADC_IBAT << 1), &val_h);
		if (ret < 0) {
			return ret;
		}

		ret = upm6722_read_byte(upm, ADC_REG_BASE + (UPM_ADC_IBAT << 1) + 1, &val_l);
		if (ret < 0) {
			return ret;
		}
		val = (val_h << 8) | val_l;
		sval = val * UPM6722_IBAT_ADC_LSB / UPM6722_IBAT_ADC_PRECISION + UPM6722_IBAT_ADC_BASE + UPM6722_IBAT_ADC_OFFSET;
		upm_err("iBat ADC correction data = %d\n", sval);
	}
...
static int upm6722_set_otg(struct upm6722 *upm ,bool en)
{
	int ret;
	u8 val;
...
static int upm6722_parse_dt(struct upm6722 *upm, struct device *dev)
{
	...
	if (of_property_read_string(np, "chg_name", &upm->chg_dev_name) < 0) {
		upm->chg_dev_name = "primary_divider_chg";
		dev_err(upm->dev, "no charger name, using default primary_divider_chg \n");
	}
...
static int mtk_upm6722_is_chg_enabled(struct charger_device *chg_dev, bool *en)
{
	...
	upm_info("enter\n");
	return upm6722_check_charge_enabled(upm, en);
}
...
static int mtk_upm6722_enable_chg(struct charger_device *chg_dev, bool en)
{
	...
	upm_info("enter\n");
	return upm6722_enable_charge(upm, en);;
}
...
static int mtk_upm6722_set_vbatovp(struct charger_device *chg_dev, u32 uV)
{
	...
	upm_info("enter\n");
	return upm6722_set_batovp_th(upm, uV / 1000);
}
...
static int mtk_upm6722_set_ibatocp(struct charger_device *chg_dev, u32 uA)
{
	...
	upm_info("enter\n");
	return upm6722_set_batocp_th(upm, uA / 1000);
}
...
static int mtk_upm6722_set_vbatovp_alarm(struct charger_device *chg_dev, u32 uV)
{
	...
	upm_info("enter\n");
	return upm6722_set_bat_ovp_alm_th(upm, uV / 1000);
}
...
static int mtk_upm6722_set_vbusovp_alarm(struct charger_device *chg_dev, u32 uV)
{
	...
	upm_info("enter\n");
	return upm6722_set_bus_ovp_alm_th(upm, CHG_SW_CAP_MODE, uV / 1000);
}
...
static int mtk_upm6722_dump_registers(struct charger_device *chg_dev)
{
	struct upm6722 *upm = charger_get_data(chg_dev);
	int rc = 0;
	int addr = 0;
	...
	pr_info("==========DUMP REG START==========\n");
	for (addr = 0x00; addr <= 0x38; addr++) {
		rc = upm6722_read_byte(upm, addr, &data);
		if (rc == 0) {
			pr_info("REG[0x%x] = 0x%x, ",addr, data);
		}
	}
	pr_info("==========DUMP REG END==========\n");
	return 0;
}
...
static const struct charger_ops upm_chg_ops = {
	...
	.dump_registers = mtk_upm6722_dump_registers,
};
                    ...
                    const union power_supply_propval *val)
{
	...
	switch (prop) {
		...
		case POWER_SUPPLY_PROP_ONLINE:
			upm6722_set_otg(upm, !!val->intval);
		break;
		default:
		return -EINVAL;
	}
...
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
	strcpy(current_cp_info.chip,"UPM6722");
	sprintf(current_cp_info.id,"0x%04x",upm->part_no);
	strcpy(current_cp_info.vendor,"Unisemi");
...
static void upm6722_charger_remove(struct i2c_client *client)
{
	struct upm6722 *upm = i2c_get_clientdata(client);
