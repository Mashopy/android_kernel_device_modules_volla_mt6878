...
//add by wanwen,aviod spi communication failure 20251011 start 
    for (int i = 0; i < 20; i++) {
...
//add by wanwen,aviod spi communication failure 20251011 end
    if (ret) {
...
//drv-add by huangxinglve for TP charger mode 20250721 start
	...
	ts_data->psy_name = "battery";
	FTS_DEBUG("fts_charger_mode_notifier_callback event psy->desc->name is %s",ts_data->psy_name);
	ts_data->charger_status = 2;
