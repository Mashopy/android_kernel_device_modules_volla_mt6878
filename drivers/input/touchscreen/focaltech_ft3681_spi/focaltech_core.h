...
//drv-add by huangxinglve for TP charger mode 20250721 end
    ...
    int edgepalm_value;
    int fwdbg_value;
    struct delayed_work fodcheck_work; //add by huangxinglve,20250606, for tp fod function
    bool wakeup_click_enabled;//add by huangxinglve,20250627, add for tp gesture setting
...
#if FTS_FOD_EN
int fts_fod_checkdown(struct fts_ts_data *ts_data);
