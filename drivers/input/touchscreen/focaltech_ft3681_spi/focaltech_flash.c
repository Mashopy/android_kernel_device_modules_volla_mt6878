...
#define FTS_FW_NAME_PREX_WITH_REQUEST               "FT368x_Ref_V01_D05_20241106_app"

//+drv-add by huangxinglve for hardware info 20250619 start
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
#include "../../../misc/mediatek/hardware_info/hardware_info.h"
extern struct hardware_info current_tp_info;
#endif
//-drv-add by huangxinglve for hardware info 20250619 end
...
static void fts_fwupg_work(struct work_struct *work)
{
    ...
    struct fts_upgrade *upg = fwupgrade;
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
    u8 fw_ver_in_tp = 0;//drv-add by huangxinglve for hardware info 20250506
#endif
    ...
    if (irq_need_recovery)
        fts_irq_enable();
    upg->ts_data->fw_loading = 0;
//+drv-add by huangxinglve for hardware info 20250506 start
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
	ret = fts_read_reg(FTS_REG_FW_VER, &fw_ver_in_tp);
	if (ret < 0) {
	        FTS_ERROR("get fw ver in tp fail");
	}
	sprintf(current_tp_info.chip,"ft3681,FW:%02x",fw_ver_in_tp);
	sprintf(current_tp_info.id,"0x01");
	strcpy(current_tp_info.vendor,"focaltech");
	strcpy(current_tp_info.more,"1080*2400");
#endif
//-drv-add by huangxinglve for hardware info 20250506 end
}
