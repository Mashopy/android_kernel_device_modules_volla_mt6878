...
extern bool panel_visionox_hc5622_fod_is_enabled(void);
...
static DEVICE_ATTR(fts_gesture_mode, S_IRUGO | S_IWUSR, fts_gesture_show,
                   fts_gesture_store);
#if FTS_FOD_EN
static DEVICE_ATTR(fts_fod_mode, S_IRUGO | S_IWUSR, fts_gesture_show,
                    fts_gesture_store);
#endif
/*
...
int fts_fod_readdata(struct fts_ts_data *ts_data)
{
	...
	FTS_DEBUG("fod_val[1]: %x: doze=%d,fod_fp_down=%d", fod_val[1],
		  panel_visionox_hc5622_fod_is_enabled(), ts_data->fod_fp_down);
	if (fod_val[1] == 0x26 && (panel_visionox_hc5622_fod_is_enabled() ||
				   ts_data->fod_fp_down)) {
		fod_pointid = fod_val[0];
		fod_x = (fod_val[4] << 8) + fod_val[5];
		fod_y = (fod_val[6] << 8) + fod_val[7];
		fod_down = (fod_val[8] == 0) ? 1 : 0;
		FTS_DEBUG("FOD data:%x %x %x %x[%x,%x][%x]", fod_val[0],
			  fod_val[1], fod_val[2], fod_val[3], fod_x, fod_x,
			  fod_val[8]);
