...
static struct pri_info g_s_pri_info;
...
static void pri_get_dts_info(struct gf_device *gf_dev)
{
	struct device_node *node = NULL;

	gf_debug(DEBUG_LOG, "%s enter\n", __func__);

	memset(&g_s_pri_info, 0, sizeof(g_s_pri_info));

	node = of_find_compatible_node(NULL, NULL, "mediatek,goodix-fp");
	if (!node) {
		pr_info("%s: get mediatek,goodix-fp fail\n",__func__);
		return;
	}

	g_s_pri_info.b_use_dts_chip_cfg = of_property_read_bool(node, "goodix-fp,use-dts-chip-info");
	if (g_s_pri_info.b_use_dts_chip_cfg) {
		of_property_read_string(node, "goodix-fp,chip-name", &g_s_pri_info.chip_name);
		if (!g_s_pri_info.chip_name) {
			pr_info("%s: get goodix-fp,chip-name fail\n",__func__);
		}

		of_property_read_u16(node, "goodix-fp,chip-id", &g_s_pri_info.chip_id);
		if (!g_s_pri_info.chip_id) {
			pr_info("%s: get goodix-fp,chip-id fail\n",__func__);
		}
	}

	pr_info("b_use_dts_chip_cfg:%d, chip_name:%s, chip_id:0x%04x", 
			g_s_pri_info.b_use_dts_chip_cfg,
			g_s_pri_info.chip_name ? g_s_pri_info.chip_name : "",
			g_s_pri_info.chip_id ? g_s_pri_info.chip_id : 0);
}
...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	gf_debug(DEBUG_LOG, "%s, regulator_get success!\n", __func__);
...
// prize durunshen for G3S set voltage  end
	...
	gf_debug(DEBUG_LOG, "%s, regulator_set_voltage 3.3v success!\n", __func__);
#endif
...
static void gf_hw_power_enable(struct gf_device *gf_dev, u8 onoff)
{
	...
	/* TODO:  set power  according to actual situation  */
		...
		if (!IS_ERR_OR_NULL(vdd_reg)){
			...
			gf_debug(ERR_LOG, "%s regulator_enable sucess!\n", __func__);
		}
#endif
		...
		if (!IS_ERR_OR_NULL(vdd_reg)){
			...
			gf_debug(ERR_LOG, "%s regulator_disable sucess!\n", __func__);
		}
#endif
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	/* pri modify for kernel-6.1 begin*/
	if (_IOC_DIR(cmd) & _IOC_READ)
		retval = !access_ok(/*VERIFY_WRITE,*/ (void __user *)arg, _IOC_SIZE(cmd));

	if (retval == 0 && _IOC_DIR(cmd) & _IOC_WRITE)
		retval = !access_ok(/*VERIFY_READ,*/ (void __user *)arg, _IOC_SIZE(cmd));
	/* pri modify for kernel-6.1 end*/
...
// drv modify macro control to report key start
#if IS_ENABLED(FP_REPORT_KEY_EVENT_SUPPORT)
...
#endif
// drv modify macro control to report key end
		break;
...
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
	sprintf(current_fingerprint_info.chip, g_s_pri_info.chip_name ? g_s_pri_info.chip_name : "GW9578");
	sprintf(current_fingerprint_info.id,"0x%x", g_s_pri_info.chip_id ? g_s_pri_info.chip_id : 0x1231);
