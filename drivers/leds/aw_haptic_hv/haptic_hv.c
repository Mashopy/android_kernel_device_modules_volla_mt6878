...
#endif
#define HAPTIC_HV_DRIVER_VERSION	"v1.6.0"
...
static int parse_dt_gpio(struct device *dev, struct aw_haptic *aw_haptic, struct device_node *np)
{
	...
	if (!(aw_haptic->flags & AW_FLAG_SKIP_INTERRUPTS)) {
...
static int read_chipid(struct aw_haptic *aw_haptic, uint32_t *reg_val)
{
	...
	*reg_val = value[0] << 8 | value[1];
	aw_info("reg_val:%x end", *(uint32_t *)reg_val);
	return ret;
}
...
static int ctrl_init(struct aw_haptic *aw_haptic)
{
	...
	for (cnt = 0; cnt < AW_READ_CHIPID_RETRIES; cnt++) {
		/* hardware reset */
		aw_info("ctrl_init hw_reset cnt:%d begin", cnt);
		hw_reset(aw_haptic);
		aw_info("ctrl_init hw_reset end");
		if (read_chipid(aw_haptic, &reg) < 0)
			aw_err("read chip id fail");
		aw_info("ctrl_init read_chipid:%x", reg);
		switch (reg) {
...
static int irq_config(struct device *dev, struct aw_haptic *aw_haptic)
{
	...
	dev_info(dev, "irq_config irq_gpio:%d\n", aw_haptic->irq_gpio);
	if (gpio_is_valid(aw_haptic->irq_gpio) &&
	    !(aw_haptic->flags & AW_FLAG_SKIP_INTERRUPTS)) {
		aw_info("irq_gpio:%d", aw_haptic->irq_gpio);
		if (gpio_is_valid(aw_haptic->irq_gpio)) {
			ret = devm_gpio_request_one(dev, aw_haptic->irq_gpio, GPIOF_DIR_IN, "aw_int");
			if (ret) {
...
static ssize_t state_aw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t state_aw_store(struct device *dev, struct device_attribute *attr,
			   const char *buf, size_t count)
{
	return count;
}

static ssize_t duration_aw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
	ktime_t time_rem;
...
static ssize_t activate_aw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
...
static DEVICE_ATTR_RW(state_aw);
static DEVICE_ATTR_RW(index);
static DEVICE_ATTR_RO(ram_num);
static DEVICE_ATTR_RW(duration_aw);
static DEVICE_ATTR_RW(activate_aw);
...
static int tiktap_file_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	vm_flags_t vm_flags = calc_vm_prot_bits(PROT_READ|PROT_WRITE, 0) |
			      calc_vm_flag_bits(file, MAP_SHARED);
...
static int vibrator_init(struct aw_haptic *aw_haptic)
{
	...
	aw_info("devm_led_classdev_register!");
...
static void haptic_init(struct aw_haptic *aw_haptic)
{
	...
	uint32_t f0_calidata = 0;
	uint32_t uefi_f0_tmp = 1700;
...
static int aw_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	if (aw_haptic == NULL) {
		pr_err("<%s>%s: devm_kzalloc nomem failed\n", AW_I2C_NAME, __func__);
		return -ENOMEM;
	}

	aw_haptic->dev = &i2c->dev;
	aw_haptic->i2c = i2c;
	aw_info("i2c_set_clientdata aw_haptic:%p", aw_haptic);
	...
	aw_info("reset_gpio:%d", aw_haptic->reset_gpio);
	if (gpio_is_valid(aw_haptic->reset_gpio)) {
	...
	aw_info("irq_gpio:%d devm_pinctrl_get ready", aw_haptic->irq_gpio);
	aw_info("irq_gpio:%d pinctrl_lookup_state", aw_haptic->irq_gpio);
	...
	aw_info("irq_gpio:%d pinctrl_select_state", aw_haptic->irq_gpio);
...
static int __init aw_i2c_init(void)
{
	...
	pr_info("<%s>%s: aw_haptic driver end\n", AW_I2C_NAME, __func__);
	return 0;
}
