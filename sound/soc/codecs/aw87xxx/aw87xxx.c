				...
				struct device_node *dev_node)
{
	...
	AW_DEV_LOGI(aw87xxx->dev, "parse dts success");
	return 0;
}
...
#endif
{
	...
	if (aw87xxx == NULL) {
		AW_DEV_LOGE(&client->dev, "malloc failed");
	...
	if (ret < 0) {
		AW_DEV_LOGE(&client->dev, "parse dts failed");
	...
	if (ret < 0) {
		AW_DEV_LOGE(&client->dev, "dev init failed");
